...
def get_data():
    ...
    extend_pandas()
    equity_rets = utils.download_returns(equity_asset)
    crypto_rets = utils.download_returns(crypto_asset)
    
    return(equity_rets,crypto_rets)

if __name__ == '__main__':
    eq_rets,cc_rets = get_data()

    # the two following should result in the output
    html(eq_rets,asset_class='equity',output='eq.html')
    html(eq_rets,output='eq.html')

    # for crypto it is compulsory to set the asset class
    html(cc_rets,asset_class='crypto',output='cc.html')
